// POST /api/wenball-result — settle a WenBall race into channel points.
//
// The game (a Godot web build at /wenball/, running as the streamer's OBS
// browser source) POSTs this when a race ends and the streamer's game URL
// carries ?results=https://wenbot.gg/api/wenball-result&token=<secret>:
//
//   { game: "wenball", channel: "<kick slug>", token: "<per-streamer secret>",
//     raceId: "<slug>-<n>-<unix>", map, length, players, winnersWanted,
//     awardPoints, raceNumber,
//     results: [ { place, username, finished, eliminated, time_ms, points, winner }, ... ] }
//
// Trust model: the game is a browser client, so nothing money-shaped is taken
// from it. The token must match the streamer's stored wenball.token (generated
// on the dashboard's WenBot Games page, same client-side pattern as
// streamdeckToken), and the payout is RE-DERIVED from the streamer's stored
// prize table — the client's "points" numbers are ignored entirely. Each
// raceId settles at most once (wenball_races/{raceId} doc written in the same
// transaction as the credits), so a replayed POST cannot pay twice.
//
// What the token can't prove is that the usernames raced: that list comes from
...
exports.handler = async (event) => {
  ...
  if (!(await checkRateLimit(db, ip, "wenball_result", 30, 60))) return res(429, { error: "Too many requests" });
  ...
  try {
    ...
    const raceRef = db.collection("streamers").doc(uid).collection("wenball_races").doc(raceId);
    const awarded = await db.runTransaction(async (tx) => {
      const existing = await tx.get(raceRef);
      if (existing.exists) throw { code: 409, msg: "Race already settled" };
      const out = [];
      for (const w of winners) {
        const pts = awardPoints ? (prizes[w.place - 1] || 0) : 0;
        if (pts > 0) {
          // Same viewer-doc shape as every other credit path (portal-daily-claim,
          // tournament payouts): lowercased doc key, kickName in display casing,
          // points via increment, merge:true so a first-time viewer doc appears.
          const vref = db.collection("streamers").doc(uid).collection("viewers").doc(w.key);
          tx.set(vref, {
            kickName:        w.username,
            points:          admin.firestore.FieldValue.increment(pts),
            lastSeen:        Date.now(),
            lastWenballWin:  Date.now(),
          }, { merge: true });
        }
      ...
      tx.set(raceRef, {
        raceId,
        map:      String(body.map || "").slice(0, 30),
    ...
    // returns, so a fire-and-forget audit write can be silently cut off (the
    // exact bug the verify-post path had). logAudit never throws.
    await logAudit(uid, "wenball_race_settled", {
      raceId,
      map: String(body.map || ""),
      players: Number(body.players) || 0,
      awarded,
    ...
    console.error("[wenball-result]", err.message || err);
