...
- **FUTURE STATE — warm `/api/portal-data` for LIVE streamers only.** Reviewed 2026-08-25 and deliberately NOT built. The 4s empty-table stall on a first portal load is a Netlify Lambda cold start (~2.3–2.7s; warm is ~0.2s). Two fixes were on the table:
  1. **Instant paint from the viewer's last payload** — BUILT (v1.38.0). Covers every returning viewer on all four portals.
  2. **A cron that pings `/api/portal-data` every ~4 min for channels whose streamer doc has `kickLive: true`** — parked. The bot already maintains `kickLive` (one batched official-API call every 2 min, see `WenBotServer/src/streamer-bot.js`), so the live list is free to read.

  **Why it was parked, not rejected.** Cost is not the objection: one compute is roughly 11 single-doc gets plus up to 29 collection gets, so ~5 live streamers pinged every 4 min over an 8-hour stream day is on the order of 60k reads/day — cents per month. The objection is that it buys very little. `portal-data` already ships `Cache-Control: public, max-age=60, stale-while-revalidate=300`, so whenever a CDN entry exists the viewer is served instantly and the cold start happens invisibly behind them. The cold start therefore only reaches someone arriving after ≥6 minutes of zero traffic — during a stream, the first viewer and nobody else — and item 1 already covers that person unless it is their first ever visit.

  **Build it if** streamers report a blank first load *during* a stream, or portals start being linked from places that send one visitor at a time (a Discord embed unfurl, a link-in-bio). Scope it to `kickLive: true` only — warming every channel unconditionally is what turns a cheap job into an expensive one.
...
- **[DONE 2026-08-20] Firestore rules deployed to logictools** — the giveaway_fairness / giveaway_draws / draw_codes / share_links locks are live and enforced. The firebase CLI on this machine has logictools access (cscogland@gmail.com); earlier belief that it only had DailyTracker was wrong. So the "seed lock not enforced" caveat is resolved. Only WenBotServer's !winner path remains outstanding from the verifiable-draw work.
